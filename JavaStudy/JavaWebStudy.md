...
- 以前开发的系统存在满足新系统功能需求的类，但其接口同新系统的接口不一致。
    例如：[[Java进阶学习/Business#新旧支付系统兼容 | 新旧支付系统兼容]]，[[Java进阶学习/Business#多数据源适配（不同数据库 / 文件格式统一读取） | 多数据源适配]] 
- 使用第三方提供的组件，但组件接口定义与自己要求的接口定义不同。
    例如：[[Java进阶学习/Business#第三方接口适配（物流查询接口统一） | 第三方接口适配]] 
...
<span style="background:#affad1">业务场景</span>：
1. 电商订单价格计算（动态优惠组合：优惠券、满减、运费、会员折扣等）
2. 接口请求增强（叠加不同的功能：日志、加密、限流等），但其实一般使用动态代理（例如AOP）即可
3. 消息通知渠道扩展（使用不同渠道发送消息：短信、微信、邮件等；不同消息格式：富文本、带附件等；消息增强：重试、签名等）
...
- 对象之间存在一对多关系，一个对象的状态发生改变会影响其他对象
    例子：[[Java进阶学习/Business#订单状态变更通知 | 订单状态变更通知]]，[[]]
